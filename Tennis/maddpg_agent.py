...
from model import Actor, Critic, Actor_Critic_Models
...
NUM_UPDATES = 4         # num of update passes when updating
...
class MADDPG():
    ...
    def __init__(self, action_size=2, seed=10, n_agents=2, noise_start=1.0, noise_decay=1.0, t_stop_noise=25000):
        """
        Params
        ======
    ...
    def step(self, all_states, all_actions, all_rewards, all_next_states, all_dones, t):
    ...
    def save_weights(self):
        ...
        for i, agent in enumerate(self.agents):
            torch.save(agent.actor_local.state_dict(),  f"checkpoint_actor{i}.pth")
            torch.save(agent.critic_local.state_dict(), f"checkpoint_critic{i}.pth")



class DDPG():
    """DDPG agent with own actor and critic that interacts with and learns from environment"""

    def __init__(self, agent_id, model, action_size=2, seed=10):
        """
        Params
        ======
            model: model object
        ...
        self.copy_weights_at_init(self.actor_target, self.actor_local)
        self.copy_weights_at_init(self.critic_target, self.critic_local)
        
        # Noise process
        self.noise = OUNoise(action_size, seed)

    def copy_weights_at_init(self, target, source):
